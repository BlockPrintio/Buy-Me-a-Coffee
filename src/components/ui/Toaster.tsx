        ...
        {toasts.map((item) => {
          ...
          return (
            <motion.div
              ...
              className="pointer-events-auto flex w-full max-w-sm items-start gap-3  bg-ink-50 p-4 shadow-plate"
            >
              <span
                className={`flex h-8 w-8 shrink-0 items-center justify-center  ${chip}`}
              >
                <Icon className="h-4 w-4" strokeWidth={2.5} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-700 text-ink-900">{item.title}</p>
                {item.description && (
              ...
              <button
                ...
                className="-mr-1 -mt-1 shrink-0  p-1.5 text-ink-400 transition-colors hover:bg-ink-100 hover:text-ink-900"
              >
                <X className="h-4 w-4" />
              </button>
            </motion.div>
